...
@section('content')
    <div class="content flex-column-fluid" id="kt_content">
        <div class="toolbar d-flex flex-stack flex-wrap mb-5 mb-lg-7" id="kt_toolbar">
            <div class="page-title d-flex flex-column py-1">
                <h1 class="d-flex align-items-center my-1"><span class="text-dark fw-bold fs-1">Cerita</span></h1>
                @include('admin.layouts._breadcrumb')
            </div>
            <div class="d-flex align-items-center py-1 gap-6">
                <button type="button" onclick="info()" class="btn btn-flex btn-sm btn-primary fw-bold border-0 fs-6 h-40px">Cerita Baru</button>
            </div>
        </div>
    ...
    <div class="modal fade" tabindex="-1" id="modal_info">
        <div class="modal-dialog modal-lg">
            <div class="modal-content" id="modal_info_ceritas">
            </div>
        </div>
    </div>
@endsection
...
@push('scripts')
    <script>
        let $form_search = $('#form_search'),
            ...
            $modal_info_ceritas = $('#modal_info_ceritas');
        let selected_page = 1, _token = '{{ csrf_token() }}', base_url = '{{ route('admin.cerita.index') }}';

        let init = () => {
            $modal_info_ceritas.html('');
        ...
        let display_modal_info = (categories) => {
            $modal_info_ceritas.html(categories);
