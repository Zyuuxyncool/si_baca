...
<head>
    ...
    <style>
        ...
        /* hide the top-right link on small screens */
    ...
    <div class="container mx-auto px-4 py-8 sm:py-12" style="padding:4rem;">
        <div class="flex items-start justify-between mb-4">
            ...
            <div class="text-right">
                <a href="{{ route('user.cerita.show', ['nama' => $cerita->nama]) }}" class="px-3 py-2 text-sm border border-gray-300 rounded-lg text-gray-200 hover:bg-gray-700 desktop-back-link">
                    Kembali ke Cerita
                </a>
            </div>
        </div>
        ...
        <a href="{{ route('user.cerita.show', ['nama' => $cerita->nama]) }}" class="mobile-back-btn" aria-label="Kembali ke Cerita">
            ...
            <div class="level-select-container col-span-full" id="level-select-example">
                ...
                <div class="level-grid">
                    ...
                    @if($ruang_tekas instanceof \Illuminate\Pagination\LengthAwarePaginator)
                        @php
                            $no = (($ruang_tekas->currentPage()-1) * $ruang_tekas->perPage()) + 1;
                        @endphp
                    @endif
                    @foreach($ruang_tekas as $ruang_teka)
                        @php
                            $poster = $ruang_teka->poster ?? $ruang_teka->photo ?? null;
                            $posterUrl = $poster ? Storage::url($poster) : null;
                            $title = $ruang_teka->title ?? $ruang_teka->nama ?? '';
                            $score = $ruang_teka->score ?? null;
                        @endphp

                        <!-- link to ruangTeka view; include template id as query param so controller can pick the template -->
                        <a href="{{ route('user.cerita.menu_ruang_teka.ruang_teka', ['nama' => $cerita->nama]) }}?id={{ $ruang_teka->id ?? '' }}" class="level-tile start-ruang-teka" data-template-id="{{ $ruang_teka->id ?? '' }}" title="{{ e($title) }}">
                            <div class="card bg-dark text-white border-0 shadow-sm level-card">
                                @if($posterUrl)
                                ...
                                <div class="card-body p-3">
                                    <div class="meta mb-2">
                                        ...
                                        <div class="score-box text-end">
                                            @if(!empty($score))
                                                ...
                                                @if(!empty($ruang_teka->last_played))
                                                    <small class="text-white" style="font-size:.68rem;display:block">Terakhir: {{ \Carbon\Carbon::parse($ruang_teka->last_played)->diffForHumans() }}</small>
                                                @endif
                                            @else
                                    ...
                                    <p class="card-text text-white small mb-0">{{ $ruang_teka->description ?? ($ruang_teka->instructions ?? '') }}</p>
                                </div>
                            </div>
                        </a>
                        @php $no++; @endphp
                ...
                <div class="d-flex flex-row justify-content-center">
                    @if($ruang_tekas instanceof \Illuminate\Pagination\LengthAwarePaginator)
                        {{ $ruang_tekas->links('vendor.pagination.custom') }}
                    @endif
                </div>
